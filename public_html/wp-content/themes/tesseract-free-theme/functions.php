...
function display_tesseractplusnotice()
{
	...
	?>
		<div id="tesseract-pro-plugin-notice" class="updated notice custom-notice <?php echo $div1_class; ?>">
			...
			$adminmfile = 'https://s3.amazonaws.com/tesseracttheme/banner/free-theme_banner.csv';
			...
			{
			    $adminmfile = get_template_directory() . '/free-theme_banner.csv';
				$csv = readCSV($adminmfile);
			}
...
function add_custom_in_menu( $items, $args )
{
    ...
    //echo "<pre>".$args->theme_location; //print_r($args);
    if($tesheadr_layout == 'centered-inline-logo' && $args->theme_location == 'primary')
    {
		...
		$menus = wp_get_nav_menu_items( $menu_obj->term_id );
		...
		$no_of_menu = count($parents);
		if($no_of_menu%2 == 0)
		{
			$position = ($no_of_menu/2);
		}
		else
		{
			$rl_position = (get_theme_mod('tesseract_centre_header_menu_rl')) ? get_theme_mod('tesseract_centre_header_menu_rl') : 'left';
			if( $rl_position == 'left')
			{
				$position = floor($no_of_menu/2);
			}
			else
			{
				$position = floor($no_of_menu/2)+1;
			}
	    ...
	    $header_font_size = (get_theme_mod('tesseract_header_upper_logo_text_size')) ? get_theme_mod('tesseract_header_upper_logo_text_size') : '34';

	    if ( $header_logo_choice == 'image' && $logoImg )
	    {
	    	$new_item = array('<li class="mob-logo-centre menu-item menu-item-type-custom menu-item-object-custom  menu-item-home"><div class="site-branding"><h1 class="site-logo site-logo2"><a href="'.esc_url( home_url( '/' ) ).'" rel="home"><img src="'.$logoImg.'" alt="logo" /></a></h1></div></li>');
	    }
	    else
	    {
	    	$new_item = array('<li class="mob-logo-centre menu-item menu-item-type-custom menu-item-object-custom  menu-item-home"><div class="site-branding"><h1 class="site-title2"><a href="'. esc_url( home_url( '/' )).'" rel="home"><span style="font-family:' .$header_fonts. '; font-size:'.$header_font_size.'px; font-style:' .$header_font_styles. '; font-weight:' .$header_font_weight.';">'.$header_text.'</span></a></h1></div></li>');
	    }
	        //$new_item       = array( '<li class="menu-item menu-item-type-post_type menu-item-object-page"><a href="/"><img src="' . get_template_directory_uri() . '/assets/img/logo.png" alt=""></a></li>' );
			
        $items          = preg_replace( '/<\/li>\s<li/', '</li>,<li',  $items );
        $array_items    = explode( ',', $items );
        array_splice( $array_items, $position, 0, $new_item ); // splice in at position 3
...
function change_logo(){
    $logo_type = get_theme_mod('tesseract_header_logo_type') ? get_theme_mod('tesseract_header_logo_type') : 'text';
    if( $logo_type == 'image' ){
        $image_url = get_theme_mod('tesseract_header_logo_image') ? get_theme_mod('tesseract_header_logo_image') : '';
        if($image_url){
            global $wpdb;
